...
Example of using Twilio to send an OTP verification code to a user and verifying the code.
...
4. Create a .env file in the root of the project and add in the above credentials from Twilio

```bash
TWILIO_ACCOUNT_SID=<ACCOUNT SID>
TWILIO_AUTHTOKEN=<AUTH TOKEN>
TWILIO_SERVICE_SID=<SERVICE ID>
```
...
```bash
curl -H "Content-Type: application/json" -X POST -d '{"phoneNumber": "+919988776655"}' http://localhost:8000/otp
```
...
```bash
curl -H "Content-Type: application/json" -X POST -d '{"user": {"phoneNumber": "+919988776655"}, "code":"795279"}' http://localhost:8000/verifyOTP
```
